...
- **数据型历史通报配方**：可按任意系统/服务历史数据生成六段式通报（总体运行/消息活跃度/模型算力/插件体系/重大事件/工作要求），每段至少 2 个长段落 + 嵌套明细，真实数字支撑、杜绝模板占位
...
### v1.1.0
- **数据型历史通报配方**：用户要求按某服务/系统历史数据生成红头文件时，按六段式结构（总体运行/消息活跃度/模型算力/插件体系/重大事件/工作要求）手动采真实数据 + `--draft` 生成；每段至少 2 个长段落 + 嵌套明细 items，杜绝单句带过/模板占位，数字粒度细到有画面感
- **移除 `scripts/generate_pdf.py`**：PDF 转换不再需要，直接交付 Word 2003 XML（WPS/Word 打开）
